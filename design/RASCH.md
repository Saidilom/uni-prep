...
- Сочинение на 24 балла — политомное задание, посчитанное **вне** модели, по
  таблице из документа. Модуль J требует его внутрь как PCM.
...
| §237 **NO CROSS-SCALE MIXING** | `averageCertificateScore` усредняет предметы в одно число. Это слой отчётности, не θ, — но требует явной пометки «composite», иначе читается как балл |
...
## Расхождения ТЗ v1.0 с решениями, принятыми до него

ТЗ теперь содержит шкалу и пороги, которых в v4.0 не было, — и в трёх местах
они расходятся с тем, что владелец решил раньше и что уже стоит в коде.
**Ничего из этого не менялось молча.** Ждут решения владельца.

### 1. Отчётный балл: 0–75 или 100?

§0.3 говорит прямо: «Шкала отчётного балла: **0–75** (максимум 75, не 100)».
Решение от 2026-09-05 — «только англ 75 бал макс остальное по 100», и так
сейчас в [certificate-scale.ts](../src/lib/certificate-scale.ts).

Противоречие не такое большое, как выглядит. Наша T-шкала **и есть** 0–75 из
ТЗ; сотенный балл — пропорциональный пересчёт поверх неё, а **буква считается
от T**, поэтому уровень A+…C от выбора шкалы не зависит вообще. Но:

- R.3 подбирает `A_scale`/`B_scale` против настоящего балла БМБА **по 75-шкале**,
  а R.5 мерит MAE «≤ 2–3 балла по 75-шкале». После линкинга главным числом
  становится 75-балльное, а сотенное — производным.
- §237 (NO CROSS-SCALE MIXING) требует, чтобы было однозначно, какая шкала
  главная. Сейчас в проекте живут обе.

Что решать: остаётся ли сотенная шкала для показа ученику (тогда 75 —
внутренняя, и это надо зафиксировать), или ученик тоже видит 0–75.
Правка сотенной шкалы сдвинет каждый показанный балл, поэтому только по
явному решению и снова через ревизии (§239).

### 2. Буква у границы: ближайшее целое или буквальные полосы?

Таблица §0.3 задаёт полосы с точностью до 0.1: `65 – 69.9 → A`,
`60 – 64.9 → B+`. Решение владельца — «если ближе к 65 баллам, то уровень тот,
что получает 65», и `gradeLevelFromScore` округляет T до ближайшего целого.

Правила расходятся на любом T в `[64.5, 65.0)`: по владельцу — `A`, по ТЗ —
`B+`. То же у каждого порога. Разница ровно в тех работах, где цена ошибки
максимальна, — L.5 про это отдельно.

**Сегодня не задевает никого**: после нулевого эталона T максимум 57.90 по
математике и 29.78 по узбекскому, ни одна из 90 работ в спорное окно не попала
(проверено — расхождений 0). Выстрелит, как только баллы поднимутся.

Отдельно: балл **ровно 70.0** не покрыт таблицей ТЗ ни одной строкой — «> 70»
и «65 – 69.9» оставляют щель. В коде `≥ 70 → A+`.

### 3. Сочинение: вне модели или PCM внутри неё?

Решение владельца — родной язык считается двумя разделами: тест по Рашу и
сочинение по 24-балльному критерию, итог — среднее арифметическое
([native-cert.ts](../src/lib/native-cert.ts)), как в `Baholash_mezoni.pdf`
стр. 4. Модуль J ТЗ помечен CORE и требует политомные задания **внутрь**
модели как PCM.

Это не обязательно спор: методика Агентства действительно описывает два
раздела, и PCM внутри первого раздела ей не противоречит. Но сочинение на 24
балла — политомное задание, и по J оно должно оцениваться порогами `τ_ik`, а не
таблицей перевода.

Сюда же **открытый вопрос про деление на два**: 54 работы по узбекскому имеют
все до одной «Ниже C» именно потому, что сочинение не написано и балл делится
на два. Без деления узбекский дал бы ≈52 вместо 26.2. Владелец отложил
(«Оставить, решу позже»); ТЗ ответа не даёт — A.3 говорит лишь, что в режиме
экзамена пропуск не даёт баллов.
...
-- Расходится ли правило буквы владельца (ближайшее целое T) с полосами ТЗ
-- §0.3. Сегодня 0: T не доходит до спорных окон. Гонять после каждого
-- изменения шкалы — это те работы, где цена ошибки максимальна (L.5).
with t as (
  select mt.subject_id, mr.grade_level as stored,
         mr.level_score * 75.0 / nullif(mr.level_score_max,0) as t_exact
  from mock_results mr join mock_tests mt on mt.id = mr.mock_test_id
  where mr.level_score is not null
), g as (
  select subject_id, stored, t_exact,
    case when round(t_exact) >= 70 then 'A+' when round(t_exact) >= 65 then 'A'
         when round(t_exact) >= 60 then 'B+' when round(t_exact) >= 55 then 'B'
         when round(t_exact) >= 50 then 'C+' when round(t_exact) >= 46 then 'C'
         else 'below_c' end as owner_rule,
    case when t_exact > 70 then 'A+' when t_exact >= 65 then 'A'
         when t_exact >= 60 then 'B+' when t_exact >= 55 then 'B'
         when t_exact >= 50 then 'C+' when t_exact >= 46 then 'C'
         else 'below_c' end as tz_rule
  from t
)
select subject_id, count(*) as works,
       count(*) filter (where owner_rule <> tz_rule) as letters_differ,
       count(*) filter (where owner_rule <> stored) as rule_vs_stored,
       round(max(t_exact)::numeric,2) as t_max
from g group by subject_id;
...
from mock_result_revisions group by reason, scale_version;
```

`rule_vs_stored = 0` — это проверка, что сохранённая буква действительно
получена из сохранённого балла действующим правилом. Ненулевое значение
означает рассинхрон `level_score` и `grade_level`, то есть балл или букву
кто-то менял в обход общего пути.
