...
## Главная проблема: показанный балл не измеряет уровень
...
1. **Каждый тест центрирует шкалу сам.** `mean(b)` — ноль до десятого знака в
   обоих тестах, то есть у каждого свой произвольный нуль. §228 (scale drift) и
   §129 (нет linking — нет сопоставимости).
2. **Балл — Z-стандартизация по когорте того же теста** (`raschThetaToT`).
   Средний T всегда ровно 50, а средний балл — всегда 66.67 из 100.

Следствия:

- математика решила 23.3% заданий и оказалась заметно ниже своего пула
  (θ = −1.836), а средний балл — **66.67**. Когорта, решившая 5%, получила бы
  **те же 66.67**;
- **прогресс между месяцами измерить нельзя** — ровно то, ради чего Rasch и
  берут;
- сильная когорта **понижает** балл каждого: число говорит о месте внутри
  теста, а выглядит как абсолютный уровень.

Это §136–137: reporting transformation отделена от θ и `score ≠ θ`. У нас балл
не является функцией одной θ — он зависит от того, кто ещё сдавал.

## Прямые попадания в запреты v4.0

| Норма | У нас |
|---|---|
| §233 **NO SILENT FALLBACK** — любой fallback configured, logged, versioned | `raschThetaToT` при вырожденной когорте молча подставляет `REFERENCE_ABILITY_STDEV = 1` и центр 0 |
...
- **Этап 1 — балл перестаёт зависеть от когорты.** Фиксированное
  преобразование θ → балл вместо Z по когорте (§136–137), плюс таблица raw →
  балл (§269). Даёт абсолютность **внутри теста**; сопоставимость между формами
  закрывает этап 4. Меняет **все уже выставленные баллы** — нужно решение:
  пересчитывать прошлое или фиксировать шкалу с новых тестов. По §239 пересчёт
  обязан создать ревизию, а не перезаписать.
