        ...
        supabase.from("placement_questions").select("*").eq("test_id", t.id).order("order").then(({ data }) => {
            if (data && data.length > 0) {
                setEditing((prev) => prev && prev.id === t.id ? {
                    ...prev,
                    questions: data.map((q: { id: string; text: string; options: Record<string, string>; correct_answer: string; points: number; order: number }) => ({
                        id: q.id,
                        text: q.text,
                        options: q.options,
                    ...
                    <div className="space-y-4">
                        ...
                        {editing.questions.map((q, idx) => (
                            <div key={q.id} className="rounded-2xl border border-border bg-muted/50 p-5 space-y-3">
                                <div className="flex items-center justify-between">
                                    <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Вопрос {idx + 1}</span>
