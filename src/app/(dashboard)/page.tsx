        ...
        <div className="flex flex-col gap-10 py-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <section className="rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
                <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">
                    {t("greeting")} <span className="font-medium text-muted-foreground">{user.name?.trim()}</span>?
                </h1>
                <p className="mt-2 max-w-xl text-sm font-normal leading-relaxed text-muted-foreground">
                    {hasPlacementResult
                        ? t("subtitleWithPlacement")
                        : t("subtitleNoPlacement")}
                </p>
            ...
            <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                <div className="grid grid-cols-2 gap-x-4 gap-y-7 sm:grid-cols-4 sm:gap-0 sm:divide-x sm:divide-border">
                    ...
                    <div className="p-6">
                        ...
                        <p className="text-xs text-muted-foreground">{t("testsCompletedLabel")}</p>
                    ...
                    <div className="p-6">
                        ...
                        <p className="text-xs text-muted-foreground">{t("testsAvailableLabel")}</p>
                    ...
                    <div className="p-6">
                        ...
                        <p className="text-xs text-muted-foreground">{t("placementTestLabel")}</p>
                        <p className={`mt-1 text-2xl font-semibold ${hasPlacementResult ? "text-emerald-600" : "text-foreground"}`}>
                            {hasPlacementResult ? t("passed") : t("notPassed")}
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section>
                <div className="mb-5 flex items-center justify-between">
                    <h2 className="text-xl font-bold tracking-tight sm:text-2xl text-[hsl(var(--brand-blue-ink))]">{t("premiumMocks")}</h2>
                ...
                {loading ? (
                    ...
                    <div className="rounded-2xl border border-border bg-muted/50 py-8 text-center dark:bg-muted/30">
                        <p className="text-sm font-medium text-muted-foreground">{t("noPaidMocksYet")}</p>
                    </div>
                ) : (
                            ...
                            return (
                                <div key={test.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-border bg-card p-5 shadow-sm transition-all hover:bg-muted/40">
                                    <div>
                                        ...
                                        <p className="mt-2 text-xs text-muted-foreground">{duration(test)} {t("minutesSuffix")}</p>
                                    </div>
                                    {status === "locked" ? (
                                        <button
            ...
            <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <div>
                    <div className="mb-5 flex items-center justify-between">
                        <h2 className="text-xl font-bold tracking-tight sm:text-2xl text-[hsl(var(--brand-blue-ink))]">{t("availableMockTests")}</h2>
                    </div>
                    {loading ? (
                        ...
                        <div className="rounded-2xl border border-border bg-muted/50 py-8 text-center dark:bg-muted/30">
                            <p className="text-sm font-medium text-muted-foreground">{t("noAvailableMocksYet")}</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {freeTests.map((test) => {
                                ...
                                return (
                                    <div key={test.id} className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm transition-all hover:bg-muted/40">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="mt-0.5 text-xs text-muted-foreground">{duration(test)} {t("minutesSuffix")}</p>
                                        </div>
                                        {status === "locked" ? (
                ...
                <div>
                    <div className="mb-5 flex items-center justify-between">
                        <h2 className="text-xl font-bold tracking-tight sm:text-2xl text-[hsl(var(--brand-blue-ink))]">{t("recentResults")}</h2>
                        {results.length > 0 ? (
                    ...
                    {loading ? (
                        ...
                        <div className="rounded-2xl border border-border bg-muted/50 py-8 text-center dark:bg-muted/30">
                            <p className="text-sm font-medium text-muted-foreground">{t("resultsWillAppearAfterFirst")}</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {recentResults.map((r) => (
                                <div key={r.id} className="flex items-center justify-between gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm">
                                    ...
                                    {/* Балл сертификата за конкретный мок, без знаменателя,
                                        как и на остальных экранах. Процент остаётся только
