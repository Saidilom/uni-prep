...
import { Plus, Trash2, Edit3, Save, X, Library, Upload } from "lucide-react";
...
export default function AdminMockTestsPage() {
    ...
    const [jsonImportSectionIdx, setJsonImportSectionIdx] = useState<number | null>(null);
        ...
        if (secs) {
            for (const sec of secs) {
                ...
                sections.push({
                    ...
                    title: sec.title,
                    order: sec.order,
                    questions: (qs || []).map((q: { id: string; text: string; options: Record<string, string>; correct_answer: string; points: number; order: number; section_id?: string; bank_id?: string | null; image_url?: string | null }) => ({ ...q, section_id: q.section_id })),
                });
            }
        }
        setEditing({
            ...
            type: t.type,
            price: t.price,
            durationMinutes: (t as unknown as { duration_minutes?: number }).duration_minutes ?? t.durationMinutes ?? 60,
            sections,
        });
    };
    ...
    const save = async () => {
        ...
        try {
            ...
            if (!testId) {
                ...
                await supabase.from("mock_tests").insert({ id: testId, ...testRow });
            } else {
                await supabase.from("mock_tests").update(testRow).eq("id", testId);
            }
    ...
    const addFromBank = (secIdx: number, q: BankQuestion) => {
        if (!editing) return;
        const qs = [...editing.sections[secIdx].questions, {
            id: crypto.randomUUID(),
            text: q.text,
            options: q.options,
            correct_answer: q.correct_answer,
            points: q.points,
            order: editing.sections[secIdx].questions.length,
            bank_id: q.id,
            image_url: q.image_url,
        }];
        const s = [...editing.sections];
        s[secIdx] = { ...s[secIdx], questions: qs };
    ...
    return (
        <div className="flex flex-col gap-10">
                ...
                <div className="rounded-3xl border border-border bg-card p-8 space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={editing.type} onChange={(e) => setEditing({ ...editing, type: e.target.value as "free" | "paid" | "class_only" })} className="w-full rounded-2xl border border-border bg-background px-4 py-3 text-foreground">
                                <option value="free">Бесплатный</option>
                                <option value="paid">Платный</option>
                                <option value="class_only">Только для класса (назначает учитель)</option>
                            </select>
                        </div>
                        <div className="space-y-1.5">
                            ...
                            <div key={sec.id || secIdx} className="rounded-2xl border border-border bg-muted/50 p-5 space-y-3">
                                ...
                                {sec.questions.map((q, qIdx) => (
                                    <div key={q.id || qIdx} className="rounded-xl border border-border bg-background p-4 space-y-2">
                                        {q.image_url && (
                                            <Image src={q.image_url} alt="" width={200} height={120} className="max-h-28 w-auto rounded-lg object-contain border border-border" />
                                        )}
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => setPickerSectionIdx(secIdx)} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:bg-muted transition-colors"><Library size={14} /> Из банка</button>
                                    <button onClick={() => setJsonImportSectionIdx(secIdx)} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:bg-muted transition-colors"><Upload size={14} /> Импорт из JSON</button>
                                </div>
                            </div>
                        ))}
                    </div>
                            ...
                            <div key={t.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-border bg-card p-5 transition-all hover:bg-muted/40 sm:flex-row sm:items-center">
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-0.5 text-xs text-muted-foreground">
                                        {t.type === "paid" ? `Платный • ${t.price} UZS` : t.type === "class_only" ? "Только для класса" : "Бесплатный"}
                                        {" • "}
                                        {(t as unknown as { duration_minutes?: number }).duration_minutes ?? t.durationMinutes ?? 0} мин
                                    </p>
                                </div>
                                <div className="flex items-center gap-2">
            ...
            {pickerSectionIdx !== null && (
                <QuestionBankPicker onAdd={(q) => addFromBank(pickerSectionIdx, q)} onClose={() => setPickerSectionIdx(null)} />
            )}
            {jsonImportSectionIdx !== null && (
                <QuestionBankJsonImport
                    onDone={(rows: ImportedBankRow[]) => rows.forEach((r) => addFromBank(jsonImportSectionIdx, r))}
                    onClose={() => setJsonImportSectionIdx(null)}
                />
            )}
        </div>
    );
