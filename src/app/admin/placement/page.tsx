...
export default function AdminPlacementPage() {
    ...
    const runImport = async () => {
        ...
        try {
            ...
            setPendingFile(null);
            toast.success("PDF распознан", { description: "Проверьте вопросы и ответы перед публикацией." });
        } catch (error) {
    ...
    const publish = async () => {
        ...
        try {
            ...
            toast.success("Тест «Школа» создан");
            setDraft(null);
            await loadTests();
        } catch (error) {
            toast.error("Не удалось опубликовать тест", { description: error instanceof Error ? error.message : String(error) });
        } finally {
            setPublishing(false);
        }
    };
    ...
    const activate = async (testId: string) => {
        ...
        try {
            ...
            toast.success("Этот тест теперь активен — все новые ученики попадут именно на него");
            await loadTests();
        } catch (error) {
            toast.error("Не удалось активировать тест", { description: error instanceof Error ? error.message : String(error) });
        } finally {
            setActivating(null);
        }
    };
    ...
    const remove = async (testId: string) => {
        if (!confirm("Удалить этот тест «Школа»?")) return;
        ...
        <div className="flex flex-col gap-10">
            <section>
                ...
                <p className="mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground sm:text-base">
                    Вступительный тест. Загрузите PDF — Gemini распознает вопросы. Ученики не назначаются вручную: активный тест открыт всем, кто отсканирует QR-код и зарегистрируется.
                </p>
            </section>
            ...
            {!pendingFile ? (
                ...
                >
                    ...
                    <span className="mt-1 text-sm text-muted-foreground">До 32 MB</span>
                </button>
            ) : (
                <div className="rounded-2xl border-2 border-dashed border-blue-300 bg-blue-50/50 p-6 dark:bg-blue-950/10">
                    ...
                    {importing && <p className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-blue-600"><Loader2 size={16} className="animate-spin" /> Это может занять пару минут</p>}
                </div>
            )}
            ...
            <section>
                ...
                {loading ? (
                    ...
                    <div className="rounded-2xl border border-border bg-muted/50 py-10 text-center dark:bg-muted/30">
                        <p className="font-medium text-muted-foreground">Пока нет тестов. Загрузите PDF выше.</p>
                    </div>
                ) : (
                    <div className="space-y-3">
                        {tests.map((t) => (
                            <div key={t.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-border bg-card p-5 sm:flex-row sm:items-center">
                                <div className="min-w-0">
                                    <div className="flex items-center gap-2">
                                        <p className="truncate font-semibold text-foreground">{t.title}</p>
                                        {t.is_active && <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-700 dark:bg-emerald-950/40"><Star size={10} /> Активен</span>}
                                    </div>
                                    <p className="mt-0.5 text-xs text-muted-foreground">{t.question_count} вопросов • Проходной: {t.passing_score}% • {t.time_limit_minutes ?? "—"} мин</p>
                                </div>
                                <div className="flex shrink-0 items-center gap-2">
                                    ...
                                    <button onClick={() => remove(t.id)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 text-red-600 hover:bg-red-50"><Trash2 size={15} /></button>
                                </div>
                            </div>
                        ))}
                    </div>
