...
function parseGeminiJson(text: string): ImportedPlacementTest {
  const trimmed = text.trim();
  const withoutFence = trimmed.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "").trim();
  const json = JSON.parse(withoutFence);
  const parsed = ImportedPlacementTestSchema.safeParse(json);
  if (!parsed.success) {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          systemInstruction: PLACEMENT_IMPORT_SYSTEM_PROMPT,
