...
// Страница группы: состав, результаты каждого ученика по каждому моку и
// список назначенных тестов.
//
// Один компонент на две роли. Супер-админ открывает её из /admin/classes,
// админ филиала — из /branch/classes; отличаются только адреса «назад» и
// ссылки на результаты мока, поэтому они и вынесены в параметры. Данные обеим
// ролям отдаёт RLS: админ филиала видит только своих учеников (миграции 072 и
// 106), и отдельной фильтрации в интерфейсе для этого не нужно.
export default function ClassDetailView({ classId, basePath, backHref }: {
    classId: string;
    /** Куда ведут ссылки на результаты мока: `${basePath}/${classId}/results/...` */
    basePath: string;
    backHref: string;
}) {
                        ...
                        {assignments.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {assignments.map((a) => (
                                    <Link
                                        key={a.id}
                                        href={`${basePath}/${classId}/results/${a.mockTestId}`}
