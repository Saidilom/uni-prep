...
describe("averageCertificateScore", () => {
    ...
    it("среднее НЕ округляется — округляет только показ (§202–203)", () => {
        // Раньше здесь стоял roundScore, и среднее выходило округлением от
        // округлённых баллов — две потери точности вместо нуля.
        // Числа подобраны так, что точное среднее НЕ кратно 0,1 — иначе тест
        // прошёл бы и при округлении и ничего бы не проверял.
        const avg = averageCertificateScore([
            { score: 49.8, max: 75 },
            { score: 50.5, max: 75 },
            { score: 51.1, max: 75 },
        ])!;
