...
6a. Whenever needsSourceImage=true, also return figureBox: the rectangle of THAT figure on its page, as fractions of the page size (x, y, width, height, each 0..1, origin at the page's top-left corner). The app crops exactly this rectangle out of the PDF and attaches it to the question as its own picture.
    - Frame the figure itself plus its caption and axis labels. Do NOT include the question text, the answer options, or any neighbouring question.
    - One page often carries several figures belonging to DIFFERENT questions (e.g. items 40a, 40b, 41a, 41b on one page). Give each question the box of its own figure only. Identical boxes on such a page mean the crop is wrong.
    - Add a small margin (about 0.01) on each side so nothing is clipped, and keep x+width and y+height within 1.
    - Return figureBox=null when the item has no figure, and also when you cannot locate the figure confidently. A guessed rectangle crops a random piece of the page, which is worse than no picture at all.
