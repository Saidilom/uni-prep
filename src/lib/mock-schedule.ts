...
// can_access_mock() Postgres function (047_manual_mock_end_time.sql)
...
export function getMockEntryState(params: {
  ...
  const { price, startsAt, endsAt, hasExistingResult, now = new Date() } = params;
  if (price <= 0 || !startsAt || !endsAt) return "unscheduled";
