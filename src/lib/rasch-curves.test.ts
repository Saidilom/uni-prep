...
import { probability, testInformation, measurementPrecision } from "./rasch";
...
describe("ICC (§D.12) — известные свойства кривой", () => {
    ...
    it("монотонно растёт по θ", () => {
        const curve = buildIcc(0.5, RANGE);
    ...
    it("остаётся в (0,1) на всём диапазоне", () => {
        const curve = buildIcc(-1.5, { min: -THETA_LIMIT, max: THETA_LIMIT });
        expect(curve.points.every((p) => p.probability > 0 && p.probability < 1)).toBe(true);
    });

    it("труднее задание — кривая правее", () => {
        const easy = buildIcc(-1, RANGE);
        const hard = buildIcc(1, RANGE);
...
describe("TCC (§D.11) — ожидаемый сырой балл", () => {
    ...
    it("растёт от 0 до числа заданий", () => {
        const tcc = buildTcc(difficulties, { min: -THETA_LIMIT, max: THETA_LIMIT });
    ...
    it("монотонна", () => {
        const tcc = buildTcc(difficulties, RANGE);
    ...
    it("при симметричных сложностях середина даёт половину заданий", () => {
        ...
        const tcc = buildTcc(symmetric, RANGE);
    ...
    it("производная TCC равна информации теста", () => {
        // Классическое тождество: dE/dθ = Σ P(1−P) = I(θ). Оно связывает две
        // кривые и ловит ошибку в любой из них.
        const range = { min: -3, max: 3 };
        const tcc = buildTcc(difficulties, range, 601);
        for (const i of [100, 300, 500]) {
            ...
            expect(slope).toBeCloseTo(testInformation(tcc.points[i].theta, difficulties), 4);
        }
    });
});
...
describe("TIF (§D.3) — информация и точность", () => {
    it("пик стоит там, где сгущены сложности", () => {
        const tif = buildTif(spread(30, 0.8, 1.2), [], RANGE);
        expect(tif.peak.theta).toBeCloseTo(1.0, 1);
    });

    it("у одного задания пик ровно на его сложности, значение 0.25", () => {
        const tif = buildTif([0.6], [], RANGE);
        expect(tif.peak.theta).toBeCloseTo(0.6, 3);
        expect(tif.peak.information).toBeCloseTo(0.25, 6);
    });
    ...
    it("SE согласована с той, что уходит в балл ученика", () => {
        ...
        const tif = buildTif(difficulties, [], RANGE);
        for (const point of [tif.points[40], tif.points[80], tif.points[120]]) {
            const fromModel = measurementPrecision(point.theta, difficulties);
            expect(point.information).toBeCloseTo(fromModel.information, 12);
            expect(point.se!).toBeCloseTo(fromModel.thetaSe!, 12);
        }
    });
    ...
    it("двугорбая информация: пик не уезжает в ложную вершину между горбами", () => {
        ...
        const tif = buildTif(twoClusters, [], RANGE);
    ...
    it("больше заданий — больше информации всюду", () => {
        const few = buildTif(spread(10, -2, 2), [], RANGE);
        const many = buildTif(spread(40, -2, 2), [], RANGE);
...
describe("нацеливание (§D.9) — то, ради чего рисуется TIF", () => {
    ...
    it("когорта в оптимуме — штрафа нет", () => {
        const tif = buildTif(difficulties, spread(20, 0.9, 1.1), RANGE);
    ...
    it("когорта в стороне — знак разрыва показывает, куда именно", () => {
        ...
        const tif = buildTif(difficulties, spread(20, -2.5, -1.5), RANGE);
    ...
    it("штраф считается именно как отношение SE, а не информации", () => {
        const tif = buildTif(difficulties, spread(20, -2, -1), RANGE);
...
describe("вклад одного задания в информацию", () => {
    it("максимум 0.25 на своей сложности", () => {
        const curve = buildItemInformationCurve(-0.8, RANGE);
        const peak = curve.reduce((best, p) => (p.information > best.information ? p : best));
        expect(peak.theta).toBeCloseTo(-0.8, 1);
        expect(peak.information).toBeLessThanOrEqual(0.25);
        expect(peak.information).toBeGreaterThan(0.24);
    });

    it("сумма вкладов равна информации теста", () => {
        const difficulties = [-1.5, -0.3, 0.4, 1.8];
        const curves = difficulties.map((b) => buildItemInformationCurve(b, RANGE));
        const tif = buildTif(difficulties, [], RANGE);
