// СЛОЖНОСТЬ ИЗ ДОЛИ РЕШИВШИХ — БОЛЬШЕ НЕ В РАСЧЁТЕ БАЛЛА.
//
// Решением владельца от 2026-09-13 балл считает 3PL (irt-3pl-calibration.ts),
// и proportionDifficulty в этой цепочке не участвует. Функция оставлена
// потому, что ею считается стартовое приближение трудности внутри самой
// калибровки 3PL, и потому, что на неё стоят тесты, описывающие поведение на
// крайних долях.
//
// cohortStatistics ниже используется по-прежнему: точка отсчёта балла
// считается по потоку независимо от того, какая модель дала θ.
//
