...
describe("raschThetaToT без когорты", () => {
    ...
    it("игнорирует переданный когортный центр, когда разброса нет", () => {
        ...
        expect(raschThetaToT(-1.0687, -1.0687, 0)).toBeCloseTo(39.313, 3);
    });

    it("возвращает T без округления", () => {
        // Главное в этой правке: T — величина промежуточная, её ещё делят между
        // разделами и переводят в шкалу предмета. Округление здесь копилось на
        // каждом следующем шаге, и балл выходил целым.
        expect(raschThetaToT(0.123, 0, 0)).toBeCloseTo(51.23, 10);
        expect(raschThetaToT(-0.4567, 0, 0)).toBeCloseTo(45.433, 10);
    });
