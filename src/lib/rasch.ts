...
/**
 * Погрешность θ, переведённая в баллы показанной шкалы.
 *
 *   T = 50 + 10·(θ − μ)/σ   ⇒   SE(T) = 10·SE(θ)/σ
 *
 * Отдельной функцией, потому что множитель здесь ровно один раз ошибиться и
 * не заметить: он выглядит как константа 10, а на деле зависит от разброса
 * потока. Пока точкой отсчёта была эталонная популяция, σ равнялась единице
 * по построению, и «·10» было верно. С центрированием по потоку (решение
 * владельца от 2026-09-11) σ стала обычным числом — на боевой математике
 * 0,82, — и прежний множитель занижал погрешность.
 *
 * `sectionCount` — сколько разделов усредняется в итоговом балле
 * (Baholash_mezoni.pdf стр. 4). У сочинения своей погрешности нет, поэтому
 * вклад измеряемого раздела делится на их число.
 *
 * null означает «погрешности не существует», а не «она нулевая» (§215, §233).
 */
export function thetaSeToScoreSe(
    thetaSe: number | null,
    cohortSigma: number,
    sectionCount = 1,
): number | null {
    if (thetaSe === null || !Number.isFinite(thetaSe) || thetaSe < 0) return null;
    if (!Number.isFinite(cohortSigma) || cohortSigma <= 0) return null;
    if (!Number.isInteger(sectionCount) || sectionCount < 1) return null;
    return (thetaSe * 10 / cohortSigma) / sectionCount;
}
...
// ═══ НИЖЕ — ИНФОРМАЦИЯ МОДЕЛИ РАША, В РАСЧЁТЕ БАЛЛА БОЛЬШЕ НЕ УЧАСТВУЕТ ═══
//
// Балл считает 3PL (irt-3pl.ts), и погрешность там своя: информация задания
// равна D²a²(P−c)²(1−P)/((1−c)²P), а не P(1−P). Эти две функции остались как
// эталон одномерной модели и для тестов; подставлять их в 3PL нельзя — они не
// знают ни дискриминации, ни угадывания и дают SE примерно на треть больше
// настоящей. И поле scoreSe у них считает по σ = 1: живой перевод в баллы
// делает thetaSeToScoreSe выше.
//
