-- Migration 033: mock_results has no "maximum possible points" figure —
-- `total_questions` is a question COUNT (submit_mock increments it once per
-- question), not a points sum, so nothing today lets the UI show "18 из 24
-- баллов". mock_answer_details.max_points is already correctly populated
-- per-question by the set_mock_answer_max_points_trg trigger (migration
-- 018), so the true per-result max is SUM(max_points) — this migration
-- persists that as a column instead of recomputing it via a join on every
-- read, and backfills every existing result from the same source.

ALTER TABLE public.mock_results
  ADD COLUMN IF NOT EXISTS max_score numeric NOT NULL DEFAULT 0;

UPDATE public.mock_results r
SET max_score = COALESCE((
  SELECT SUM(mad.max_points) FROM public.mock_answer_details mad WHERE mad.result_id = r.id
), 0);

-- submit_mock: same as migration 023's version, plus a v_max_score
-- accumulator (points possible per question, not just points earned)
-- summed alongside v_score, stored on the new column, and returned to the
-- caller as 'maxScore' so the just-submitted result screen doesn't need a
