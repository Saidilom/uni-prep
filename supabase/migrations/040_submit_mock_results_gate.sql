-- #5: when a paid mock has results_publish_at set in the future, the
-- student's own submit_mock response must not leak score/percentage/level
-- data before the announced date — everything is still scored and stored
-- normally (teacher/admin views, Rasch recalculation, AI essay grading are
-- all unaffected), only what's handed back to the submitting student's own
-- browser is redacted. Access-window enforcement itself already happens for
-- free via the existing can_access_mock(p_mock_test_id) check below (see
-- 039_can_access_mock_schedule_window.sql) — nothing else changes here.
...
BEGIN
  ...
  IF v_mock_test.price > 0 AND v_mock_test.results_publish_at IS NOT NULL AND now() < v_mock_test.results_publish_at THEN
